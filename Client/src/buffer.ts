...
export class Buffer {
  ...
  protected update(vertices) {
    ...
    this.program.vertexPositionAttribute = this.gl.getAttribLocation(
      this.program,
      "position"
    );
  ...
  protected createProgram(
    ...
    this.gl.shaderSource(vShader, vshaderScript!);
    this.gl.compileShader(vShader);

    let fShader = this.gl.createShader(this.gl.FRAGMENT_SHADER);
    this.gl.shaderSource(fShader!, fshaderScript!);
    ...
    if (isOcean) {
      ...
      this.program.spectrum = this.gl.getUniformLocation(
        this.program,
        "displacement"
      );
      this.program.reflection = this.gl.getUniformLocation(
        this.program,
        "reflectionSampler"
      );
      this.program.refraction = this.gl.getUniformLocation(
        this.program,
        "refractionSampler"
      );
    } else {
      ...
      this.program.clipPlane = this.gl.getUniformLocation(
        this.program,
        "clipplane"
      );
      this.program.isclipped = this.gl.getUniformLocation(
        this.program,
        "isclipped"
      );
    }

    this.program.cameraPosition = this.gl.getUniformLocation(
      this.program,
      "cameraPosition"
    );
    ...
    this.program.projectionMatrix = this.gl.getUniformLocation(
      this.program,
      "projMatrix"
    );
    this.program.viewMatrixMatrix = this.gl.getUniformLocation(
      this.program,
      "viewMatrix"
    );
    this.program.invViewMatrix = this.gl.getUniformLocation(
      this.program,
      "invViewMatrix"
    );
    this.program.invProjMatrix = this.gl.getUniformLocation(
      this.program,
      "invProjMatrix"
    );
    this.program.birdviewMatrix = this.gl.getUniformLocation(
      this.program,
      "birdviewMatrix"
    );
